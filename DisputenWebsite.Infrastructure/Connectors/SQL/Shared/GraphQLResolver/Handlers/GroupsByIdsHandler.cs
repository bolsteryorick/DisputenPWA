...
{
    public class GroupsByIdsHandler : IRequestHandler<GroupsByIdsRequest, GroupsByIdsResult>
    {
        ...
        public async Task<GroupsByIdsResult> Handle(GroupsByIdsRequest request, CancellationToken cancellationToken)
        {
            return new GroupsByIdsResult(
                await ResolveGroupsByIds(request.GroupIds, request.Helper, cancellationToken)
                );
        }
        ...
        {
            var appEvents = (await _mediator.Send(new AppEventsFromGroupIdsRequest(groupIds, helper.AppEventPropertyHelper), cancellationToken)).Result;
        ...
        {
            var members = (await _mediator.Send(new MembersByGroupIdsRequest(groupIds, helper.MemberPropertyHelper), cancellationToken)).Result;
